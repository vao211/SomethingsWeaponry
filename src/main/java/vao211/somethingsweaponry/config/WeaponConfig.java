package vao211.somethingsweaponry.config;

import eu.midnightdust.lib.config.MidnightConfig;

public class WeaponConfig extends MidnightConfig {
    public static final String MOD_ID = "somethingsweaponry";
    @Entry(category = "weapons")
    public static float oblivionSwordDamage = 8.0f;
    @Entry(category = "weapons")
    public static float oblivionSwordAttackSpeed = 1.5f;
    @Entry(category = "weapons")
    public static float oblivionDaggerDamage = 6.0f;
    @Entry(category = "weapons")
    public static float oblivionDaggerAttackSpeed = 2.0f;
    @Entry(category = "weapons")
    public static float oblivionAxeDamage = 10.0f;
    @Entry(category = "weapons")
    public static float oblivionAxeAttackSpeed = 1.0f;
    @Entry(category = "weapons")
    public static float oblivionBladeDamage = 9.0f;
    @Entry(category = "weapons")
    public static float oblivionBladeAttackSpeed = 1.3f;
    @Entry(category = "weapons")
    public static float oblivionHeavyAxeDamage = 13.0f;
    @Entry(category = "weapons")
    public static float oblivionHeavyAxeAttackSpeed = 0.8f;
    @Entry(category = "weapons")
    public static float oblivionHeavySwordDamage = 11.0f;
    @Entry(category = "weapons")
    public static float oblivionHeavySwordAttackSpeed = 1.0f;
    @Entry(category = "weapons")
    public static float oblivionBattleHammerDamage = 12.0f;
    @Entry(category = "weapons")
    public static float oblivionBattleHammerAttackSpeed = 0.9f;
    @Entry(category = "weapons")
    public static float oblivionSpearDamage = 8.0f;
    @Entry(category = "weapons")
    public static float oblivionSpearAttackSpeed = 1.3f;
    @Entry(category = "weapons")
    public static float oblivionTridentDamage = 10.0f;
    @Entry(category = "weapons")
    public static float oblivionTridentAttackSpeed = 1.1f;
    @Entry(category = "weapons")
    public static float oblivionHalberdDamage = 12.0f;
    @Entry(category = "weapons")
    public static float oblivionHalberdAttackSpeed = 0.9f;

}